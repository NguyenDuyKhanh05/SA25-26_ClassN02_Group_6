...
const Marketing = () => {
    ...
    const [promos, setPromos] = useState([
        { id: 1, code: 'HAPPYHOUR', title: 'HAPPY HOUR', desc: 'Mua 1 Tặng 1 Espresso', time: '7h - 9h hàng ngày', color: 'from-orange-500 to-red-500', used: 145, limit: 200, status: 'running', revenue: 5600000 },
        { id: 2, code: 'VIP20', title: 'MEMBER DAY', desc: 'Giảm 20% cho hạng Vàng', time: 'Thứ 5 hàng tuần', color: 'from-yellow-400 to-amber-600', used: 32, limit: 1000, status: 'running', revenue: 3200000 },
        { id: 3, code: 'MORNING', title: 'COMBO SÁNG', desc: 'Cafe + Bánh mì 49k', time: 'Trước 10h sáng', color: 'from-blue-400 to-indigo-600', used: 890, limit: 1000, status: 'expired', revenue: 12500000 },
        { id: 4, code: 'NEWYEAR', title: 'KHỞI ĐẦU MỚI', desc: 'Đồng giá 29k Cafe Máy', time: '13h - 15h T1/2025', color: 'from-emerald-400 to-teal-600', used: 0, limit: 500, status: 'upcoming', revenue: 0 },
    ]);
    ...
    const handleSave = (e) => {
        ...
        if (editingPromo) {
            // Logic Cập nhật
            setPromos(promos.map(p => p.id === editingPromo.id ? { ...p, ...formProps } : p));
        } else {
            // Logic Tạo mới
            const newPromo = {
                id: Date.now(),
        ...
        <div className="w-full h-full p-[30px] animate-fadeIn overflow-y-auto relative bg-bg">
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-6">
                {filteredPromos.map((p) => (
                    <div key={p.id} className={`relative bg-surface rounded-2xl border border-white/5 overflow-hidden group hover:-translate-y-1 transition-transform duration-300 ${p.status === 'expired' ? 'opacity-60 grayscale' : ''}`}>
                        ...
                        <div className="p-5 pt-6 relative">
                            ...
                            <div className="bg-bg p-3 rounded-lg border border-white/5 mb-4 flex justify-between items-center group/code">
                                ...
                                <button 
                                    ...
                                    className={`text-xs px-3 py-1 rounded font-bold transition-all ${copiedId === p.id ? 'bg-success text-white' : 'bg-white/10 hover:bg-white/20 text-white'}`}
                                >
                            ...
                            <div className="absolute inset-0 bg-surface/90 backdrop-blur-sm flex flex-col items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                ...
                                {p.status === 'running' && (
                                    <button onClick={() => handleStop(p.id)} className="w-[120px] py-2 bg-warning text-black font-bold rounded-lg hover:bg-yellow-400 shadow-lg transform hover:scale-105 transition-all">
                                        ⏸ Tạm dừng
                                    </button>
                                )}
                                <button onClick={() => handleDelete(p.id)} className="w-[120px] py-2 bg-danger text-white font-bold rounded-lg hover:bg-red-600 shadow-lg transform hover:scale-105 transition-all">
                                    🗑️ Xóa bỏ
                                </button>
                            </div>
                            {/* 👆👆👆 -------------------------------------- 👆👆👆 */}
